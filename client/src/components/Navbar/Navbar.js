          ...
          <ul className="mb-4">
            {navlinks.map((link, idx) => {
              return (
                <li
                  key={`${link.name}-${idx}`}
                  onClick={() => onListClickHandler(link)}
                  className={`flex p-4 ${
                ...
                >
                  <img
                    ...
                    className={`w-[24px] h-[24px] object-contain ${
                      isNavItem(link, isActive) ? "grayscale-0" : "grayscale"
                    }`}
                  />
                  <p
                    className={` ml-[20px] font-epilogue text-[14px] ${
                      isNavItem(link, isActive)
                        ? "text-[#1dc071]"
                        : "text-[#808191]"
                    } capitalize`}
                  >
